...
struct ScoreFunction
{
    ...
    score_engine::ScoreEngine<numberOfInputNeurons,
        numberOfOutputNeurons,
        numberOfTicks,        
        numberOfNeighbors,    
        populationThreshold,  
        numberOfMutations,    
        solutionThreshold > _computeBuffer[solutionBufferCount];
    ...
    m256i getLastOutput(const unsigned long long processor_Number)
    {
        ...
        m256i result = m256i::zero();
        _computeBuffer[processor_Number].getLastOutput(result.m256i_u8, 32);
