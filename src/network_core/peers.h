...
static int numberOfAcceptedConnection = 0;
...
static void closePeer(Peer* peer)
{
    if (((unsigned long long)peer->tcp4Protocol) > 1)
    {
        ...
        if (peer->isConnectedAccepted)
        {
            numberOfAcceptedConnection--;
            ASSERT(numberOfAcceptedConnection >= 0);
        }
...
static bool peerConnectionNewlyEstablished(unsigned int i)
{
        ...
        && peers[i].connectAcceptToken.CompletionToken.Status != -1)
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (NUMBER_OF_INCOMING_CONNECTIONS + NUMBER_OF_OUTGOING_CONNECTIONS - numberOfAcceptedConnection < NUMBER_OF_PRESERVE_SLOTS_WHITE_LIST_IPS)
                        {
        ...
        {
            numberOfAcceptedConnection++;
            ASSERT(numberOfAcceptedConnection <= NUMBER_OF_INCOMING_CONNECTIONS + NUMBER_OF_OUTGOING_CONNECTIONS);
            return true;
        }
    }
    return false;
