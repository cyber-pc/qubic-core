...
static long long save2(const CHAR16* fileName, unsigned long long totalSize, const unsigned char* buffer, const CHAR16* directory = NULL, EFI_FILE_PROTOCOL* rootFilePointer = NULL)
{
#ifdef NO_UEFI
    logToConsole(L"NO_UEFI implementation of save() is missing! No file saved!");
    return 0;
    ...
    if (NULL != rootFilePointer)
    {
        rootFile = rootFilePointer;
    }
    else
    {
        rootFile = root;
    ...
    if (NULL != directory)
    {
        ...
        // Open the directory
        if (status = rootFile->Open(rootFile, (void**)&directoryProtocol, (CHAR16*)directory, EFI_FILE_MODE_READ, 0))
        {
            logStatusToConsole(L"FileIOSave:OpenDir EFI_FILE_PROTOCOL.Open() fails", status, __LINE__);
            return -1;
        }
    ...
    {
        if (status = rootFile->Open(rootFile, (void**)&file, (CHAR16*)fileName, EFI_FILE_MODE_READ | EFI_FILE_MODE_WRITE | EFI_FILE_MODE_CREATE, 0))
        {
            logStatusToConsole(L"FileIOSave:OpenFile EFI_FILE_PROTOCOL.Open() fails", status, __LINE__);
            return -1;
        }
...
static bool initFilesystem2(EFI_FILE_PROTOCOL* pRootFile)
{
    pRootFile = NULL;
    EFI_SIMPLE_FILE_SYSTEM_PROTOCOL* simpleFileSystemProtocol = NULL;
    ...
    if (status = simpleFileSystemProtocol->OpenVolume(simpleFileSystemProtocol, (void**)&pRootFile))
    {
        logStatusToConsole(L"EFI_SIMPLE_FILE_SYSTEM_PROTOCOL.OpenVolume() fails", status, __LINE__);
