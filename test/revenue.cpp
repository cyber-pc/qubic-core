...
TEST(TestCoreRevenue, V2BonusFactorCombination)
{
    ...
    // Both XMR and DOGE full -> E = S -> revenue = IPC (100%)
    resetData();
    for (unsigned int i = 0; i < NUMBER_OF_COMPUTORS; i++)
    {
    ...
    // XMR full, DOGE zero -> E = S/2 -> revenue = IPC * S * (S^2 + B*S/2) / DIVISOR
    resetData();
    for (unsigned int i = 0; i < NUMBER_OF_COMPUTORS; i++)
    {
        gRevenueComponents.customMiningScore[i] = 1000;
    ...
    // XMR zero, DOGE full -> same as above (symmetric)
    resetData();
    for (unsigned int i = 0; i < NUMBER_OF_COMPUTORS; i++)
    {
    ...
    // Both zero -> E = 0 -> revenue = IPC * S/(S+B) ~ 80%
    resetData();
    for (unsigned int i = 0; i < NUMBER_OF_COMPUTORS; i++)
    {
        gRevenueComponents.customMiningScore[i] = 0;
